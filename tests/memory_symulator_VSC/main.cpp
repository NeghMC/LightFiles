...
uint8_t singleWriteTest()
{
    ...
    result = lf_write(key, bufferIn, dataSize);
    if(result != LF_RESULT_SUCCESS){return 1;}
...
uint8_t multipleWritesAndReadsTest()
{
    ...
    {
        result = lf_write(i, testData[i].bufferIn, dataSize);
        if(result != LF_RESULT_SUCCESS){return 1;}
    }
    ...
    result = lf_write(3, testData[3].bufferIn, dataSize);
    if(result != LF_RESULT_SUCCESS){return 1;}
    ...
    result = lf_read(3, bufferOut, dataSize);
    if(result != LF_RESULT_SUCCESS){return 1;}
