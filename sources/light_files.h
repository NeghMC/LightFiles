...
lf_result_t lf_write(void *content, uint16_t length); // writes data
lf_result_t lf_save(void); // ends writing mode

// read
lf_result_t lf_open(uint8_t key); // starts reading mode
lf_result_t lf_read(void *content, uint16_t length); // reads data
...
lf_result_t lf_app_init(uint16_t *blockCount, uint16_t *blockSize);
    // shall store block count and block size under the provided pointers
